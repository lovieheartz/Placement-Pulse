...
const HODNotificationHistory = () => {
  ...
  return (
    <PortalLayout role="hod" title="Notification History" user={profileData || user}>
          <div className="w-full">
            {/* Header */}
            <PageHeader
              title="Notification History"
              subtitle="View and search all your sent notifications"
              icon={History}
            />
            ...
            <GlassPanel className="p-4 mb-6">
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent px-4 py-2 border-0 focus:ring-0 focus:outline-none text-foreground placeholder:text-muted-foreground"
                />
                {searchTerm && (
                  <button
                    onClick={() => {
                    ...
                    className="p-2 hover:bg-accent/40 rounded-full transition-colors"
                  >
            ...
            {isLoading ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-muted-foreground">Loading notification history...</p>
              </div>
                ...
                <GlassPanel className="p-0 overflow-hidden">
                  {historyData?.data?.map((notification, index) => (
                    <div key={notification._id}>
                      <div
                        className="flex items-center px-6 py-4 hover:bg-accent/40 cursor-pointer border-b border-border/60 transition-colors"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {notification.attachment && (
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-muted-foreground flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                              </svg>
                            )}
                          </div>
                          <p className="text-sm text-muted-foreground truncate mb-1">{notification.description}</p>
                        ...
                        <div className="flex items-center gap-4 ml-4 flex-shrink-0">
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-muted-foreground transition-transform ${selectedNotification?._id === notification._id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      {selectedNotification?._id === notification._id && (
                        <div className="px-6 py-4 bg-muted/40 border-b border-border/60">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Recipients */}
                            <div>
                              <h4 className="text-sm font-semibold text-foreground mb-2">Recipients</h4>
                              <p className="text-sm text-muted-foreground">{getRecipientSummary(notification.recipients)}</p>
                              {notification.recipients.emails && notification.recipients.emails.length > 0 && (
                                <div className="mt-2">
                                  <p className="text-xs font-medium text-foreground mb-1">Direct Emails:</p>
                            ...
                            <div className="space-y-3">
                              {notification.formLink && (
                                <div>
                                  <h4 className="text-sm font-semibold text-foreground mb-1">Form Link</h4>
                                  <a href={notification.formLink} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline break-all">
                                    {notification.formLink}
                                  </a>
                                </div>
                              )}
                              {notification.deadline && (
                                <div>
                                  <h4 className="text-sm font-semibold text-foreground mb-1">Deadline</h4>
                                  <p className="text-sm text-destructive">{formatDate(notification.deadline)}</p>
                                </div>
                              )}
                              {notification.attachment && (
                                <div>
                                  <h4 className="text-sm font-semibold text-foreground mb-1">Attachment</h4>
                                  <p className="text-sm text-muted-foreground">{notification.attachment.filename}</p>
                                </div>
                              )}
                              {notification.extraInfo && (
                                <div>
                                  <h4 className="text-sm font-semibold text-foreground mb-1">Additional Info</h4>
                                  <p className="text-sm text-muted-foreground">{notification.extraInfo}</p>
                                </div>
                              )}
                            </div>
                          </div>
...
export default HODNotificationHistory;
